...
class BlobsS3ClientV1 {
    ...
    createBlobFromUri(correlationId, blob, uri, callback) {
        blob.id = blob.id || pip_services_commons_node_4.IdGenerator.nextLong();
    ...
    createBlobFromData(correlationId, blob, buffer, callback) {
        ...
        let params = {
            ...
            Expires: pip_services_commons_node_7.DateTimeConverter.toNullableDateTime(blob.expire_time),
            Metadata: {
                name: blob.name || blob.id,
                group: blob.group || "",
                completed: pip_services_commons_node_5.StringConverter.toString(blob.completed)
            },
            Body: buffer
        };
